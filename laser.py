...
# --- PULSANTE DI STRUTTURA PER IL CARICAMENTO AUTOMATICO DI TUTTI I DATI DELLE FOTO ---
st.markdown("### 📥 Ripristino Dati da Foto")
if st.button("🚀 CARICA AUTOMATICAMENTE TUTTI I PASSAGGI DELLE FOTO", type="primary", use_container_width=True):
    tutto_foto = [
        # --- FOTO 1: STELLA, STARLETTE E LARES ---
        ...
        # --- FOTO 3: LAGEOS 1, LAGEOS 2 E LARES 2 ---
    ...
    with sqlite3.connect("laser_data_v2.db") as c:
        c.cursor().executemany("INSERT INTO acquisitions VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?)", tutto_foto)
        c.commit()
    st.success("📊 Tutti i passaggi storici delle 3 foto sono stati importati con successo!")
    st.rerun()
